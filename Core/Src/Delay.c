...
static uint16_t fac_s_conut=0;//Sec count
...
{
	uint32_t temp;
	SysTick->LOAD=(uint32_t)1000*fac_ms;//時間載入(SysTick->LOAD為24bit)
	SysTick->VAL =0x00;           //清空計數器
	SysTick->CTRL|=SysTick_CTRL_ENABLE_Msk;          //開始倒數
	while(fac_s_conut<ns)//等待時間到達
	{
		do
		{
			temp=SysTick->CTRL;
...
void TM_Delay_Init(void) {
	...
	multiplier = 80000000 / 4000000;//STM32L4A6 =80MHZ
}
